...
> - **ClassIsland 客户端「精确时间」设置只接受标准 NTP 端口 123。**如果将本插件改为其他端口，客户端将无法识别。

> [!NOTE]
> - 插件加载后自动启动 NTP 服务，无需手动操作。
> - 支持自定义监听端口，但非特权端口（>1024）只对自定义客户端有效，无法被 ClassIsland 客户端识别。
...
4. 在其他 ClassIsland 端的「精确时间」设置中填入本机 IP（**不要带端口**）即可同步时间
